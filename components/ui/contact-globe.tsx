...
type GeometryObject = { type: string; coordinates: unknown[] };
type GeometryCollection = { type: string; geometries: GeometryObject[] };
...
type CustomProjection = d3.GeoProjection & { alpha?: (value?: number) => number | d3.GeoProjection };
...
function interpolateProjection(
  ...
  let t = 0;
  const createRawProjection = (alpha: number) => {
...
function GlobeWireframe({
  width, height, className = "aspect-square w-full max-w-150",
  strokeColor = "currentColor", strokeWidth = 1.0,
  graticuleColor = "currentColor", graticuleOpacity = 0.2,
  sphereOutlineColor = "currentColor", sphereOutlineWidth = 1,
  autoRotate = true, autoRotateSpeed = 0.5,
  rotateToLocation, rotateCities = [],
  rotationSpeed = 3000, initialRotation = [0, 0],
  enableInteraction = true, showGraticule = true,
  animationDuration = 2000, startAsGlobe = true,
  countryFillColor, countryHoverColor,
  variant = "wireframe", scale = 1, backgroundColor,
}: GlobeWireframeProps) {
  ...
  useEffect(() => {
    ...
    const updateDimensions = () => {
      if (container && useResponsive) {
        setDimensions({ width: container.offsetWidth || 300, height: container.offsetWidth || 300 });
      }
    };
    updateDimensions();
    if (useResponsive) {
  ...
  useEffect(() => {
    const loadWorldData = async () => {
      try {
        ...
        const countries = feature(world, world.objects.countries) as any as { features: GeoFeature[] };
        setWorldData(countries.features);
      } catch (error) {
        setWorldData([{
          type: "Feature",
          geometry: { type: "Polygon", coordinates: [[[-180, -90], [180, -90], [180, 90], [-180, 90], [-180, -90]]] } as GeometryObject,
          properties: {},
        }]);
      }
    };
  ...
  useEffect(() => {
    ...
    const rotateToNextCity = () => {
      ...
      if (coordinates) { animateRotationTo([-coordinates[1], -coordinates[0]], rotationSpeed * 0.6); setCurrentCityIndex(nextIndex); }
    };
  ...
  useEffect(() => {
    ...
    const finalCountryFill = variant === "solid" ? defaultCountryFillColor : "none";
    const finalStrokeWidth = strokeWidth;
    const finalOpacity = 1.0;
    const renderGraticule = showGraticule;
    const finalGraticuleOpacity = graticuleOpacity;
    const finalSphereOutlineWidth = sphereOutlineWidth;
    ...
    if (variant === "wireframe") {
      ...
      projection = interpolateProjection(orthographicRaw, equirectangularRaw)
        .scale(scaleRange(alpha)).translate([finalWidth / 2, finalHeight / 2])
        .rotate([baseRotate(alpha) + rotation[0], rotation[1]]).precision(0.1);
      // @ts-ignore
      ...
      try {
        ...
        if (graticulePath) {
          svg.append("path").datum(graticule()).attr("d", graticulePath)
            .attr("fill", "none").attr("stroke", defaultGraticuleColor)
            .attr("stroke-width", 1).attr("opacity", finalGraticuleOpacity);
        }
      } catch (error) { console.error("Error creating graticule:", error); }
    }

    svg.selectAll(".country").data(worldData).enter().append("path")
      .attr("class", "country").attr("d", (d: GeoFeature) => {
        try {
          const pathString = path(d as unknown as GeoPermissibleObjects);
          if (!pathString || (typeof pathString === "string" && (pathString.includes("NaN") || pathString.includes("Infinity")))) return "";
          return pathString;
        } catch { return ""; }
      })
      .attr("fill", finalCountryFill).attr("stroke", defaultStrokeColor)
      .attr("stroke-width", finalStrokeWidth).attr("opacity", finalOpacity)
    ...
    try {
      ...
      if (sphereOutline) {
        svg.append("path").datum({ type: "Sphere" }).attr("d", sphereOutline)
          .attr("fill", "none").attr("stroke", defaultSphereOutlineColor)
          .attr("stroke-width", finalSphereOutlineWidth)
          .attr("opacity", variant === "wireframe" ? 1.0 : 0.8);
      }
    } catch (error) { console.error("Error creating sphere outline:", error); }
  }, [worldData, progress, rotation, isVisible, finalWidth, finalHeight,
    defaultStrokeColor, strokeWidth, defaultGraticuleColor, graticuleOpacity,
    defaultSphereOutlineColor, sphereOutlineWidth, showGraticule,
    defaultCountryFillColor, countryHoverColor, variant, scale,
    defaultBackgroundColor, useResponsive, dimensions.width]);

  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <svg ref={svgRef} width={finalWidth} height={finalHeight}
...
>(({ className, orientation = "horizontal", decorative = true, ...props }, ref) => {
  ...
  return (
    <SeparatorPrimitive.Root ref={ref} decorative={decorative} orientation={orientation}
      className={cn("shrink-0 flex items-center justify-center overflow-hidden", isHorizontal ? "w-full" : "h-full", className)} {...props}>
      <div className={cn("relative", isHorizontal ? "w-full h-4" : "h-full w-4")}>
        <div className={cn("absolute inset-0 bg-repeat", "text-neutral-400 dark:text-white/20")}
          style={{
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold tracking-widest uppercase text-[rgba(175,0,0,0.5)]">Full Name</label>
                <input type="text" placeholder="Your Name"
                  className="w-full bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-zinc-800 dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 outline-none focus:border-[rgba(175,0,0,0.5)] dark:focus:border-[rgba(175,0,0,0.5)] focus:ring-2 focus:ring-[rgba(175,0,0,0.1)] transition-all duration-200" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold tracking-widest uppercase text-[rgba(175,0,0,0.5)]">Company</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold tracking-widest uppercase text-[rgba(175,0,0,0.5)]">Email Address</label>
              <input type="email" placeholder="you@example.com"
                className="w-full bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-zinc-800 dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 outline-none focus:border-[rgba(175,0,0,0.5)] dark:focus:border-[rgba(175,0,0,0.5)] focus:ring-2 focus:ring-[rgba(175,0,0,0.1)] transition-all duration-200" />
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold tracking-widest uppercase text-[rgba(175,0,0,0.5)]">Message</label>
              <textarea placeholder="Type your message here" rows={4}
                className="w-full bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700 rounded-xl px-4 py-3 text-sm text-zinc-800 dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 outline-none focus:border-[rgba(175,0,0,0.5)] dark:focus:border-[rgba(175,0,0,0.5)] focus:ring-2 focus:ring-[rgba(175,0,0,0.1)] resize-none transition-all duration-200" />
            </div>

            <Button className="w-fit h-11 px-8 rounded-xl font-semibold text-sm bg-[rgba(175,0,0,1)] hover:bg-[rgba(175,0,0,0.9)] text-white group">
              Submit <ArrowRight className="w-4 h-4 transition-transform duration-200 group-hover:translate-x-1" />
            </Button>
          </motion.div>
        </div>
      </div>
